...
varis_selected = [str for str in varis_selected if any(sub in str for sub in ['High','Low'])==False]
...
dfpp.plot(color='Category', alpha=0.3, fill_between=True, final_ranges=False,
...
dfpp = dfp.filter(variable=indi, 
                  # model='AIM/CGE 2.1', scenario='CO_Bridge',
                  Category=['C3',], region=iso)
...
dfpp.plot(color='region', alpha=0.3, fill_between=True, final_ranges=False,
...
dfe = dfar6.filter(variable='AR6 climate diagnostics|Surface Temperature (GSAT)|MAGICCv7.5.3|50.0th Percentile',
                   Category=['C1','C3','C8'])
#%%
...
dfe.plot(color='Category', legend=False, fill_between=True, final_ranges=False,
         title='AR6DB - GSAT p50 - MAGICC',
         ax=ax, alpha=0.4,  order=['year'])
...
#%% by emissions
dfe = dfar6.filter(variable='AR6 climate diagnostics|Infilled|Emissions|CO2',
                    Category=['C1','C3','C8'])
...
dfe.plot(color='Category', legend=False, fill_between=True, final_ranges=True,
