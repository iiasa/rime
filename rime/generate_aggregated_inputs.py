...
for i, ind in enumerate(zip(indicators, files)):
    ...
    df = dfin.interpolate(time=years,)
    ...
    with alive_bar(total=len(small_vars),
        ...
        for vari in small_vars:
            ...
            dfp = df_ind.melt(id_vars=['model','scenario','variable','region','unit', 'SSP', 'GMT'],
                       value_vars=years, var_name='year') # change to df_big if concatenating multiple
            ...
            for indicator in dfp.variable.unique():
                print(indicator)
                dx = dfp.loc[dfp.variable==indicator].set_index(['gmt','year','ssp','region']).to_xarray()
