...
dask.config.set(num_workers=72)
...
mapdata = xr.open_mfdataset(files, preprocess=preprocess, combine='nested', concat_dim = 'gmt')

# df = pyam.IamDataFrame(dft)

map_out_MS = map_transform_gmt_multi_dask(dft.filter(model='POLES ADVANCE'), mapdata, use_dask=True, gmt_name='threshold')
...
ssp = 'ssp2'
mapdata = xr.Dataset()
indicators = ['cdd', 'precip', 'dri','dri_qtot','ia_var', 'ia_var_qtot','sdd_18p3', 'sdd_24p0'] #'heatwave']
...
for ind in indicators:
    ...
    for var in params['indicators'][ind]: 
        ...
        short = params['indicators'][ind][var]['short_name']
        files = glob.glob(os.path.join(impact_data_dir, ind, f'*{short}_{ssp}*{ftype}.nc4'))        
        mapdata[short] = xr.open_mfdataset(files, preprocess=preprocess, combine='nested', concat_dim = 'gmt')[short]

map_out_MI = map_transform_gmt_multi_dask(dft.filter(model='AIM*', scenario='SSP1-34'), mapdata, use_dask=False,  gmt_name='threshold')
...
#def test_map_transform_gmt():   
    #return x


#def test_map_transform_gmt_multi():
