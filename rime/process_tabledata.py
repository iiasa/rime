...
#%%
    ...
    for year_res in year_resols:
        for f in files:
            ...
            dft = np.round(dft.as_pandas()[pyam.IAMC_IDX+['year','value','Ssp_family']], 3)
            ...
            if parallel:
                ...
                outd = ddf.apply(calculate_impacts_gmt, dsi=dsi,  axis=1, meta=('result', None))
                # outdd = client.map(ddf.apply(calculate_impacts_gmt, dsi=dsi,  axis=1))
                
                with ProgressBar():
                    # try:
                        df_new = outd.compute(num_workers=num_workers)#scheduler='processes')
                        print(f' Applied:  {time.time()-start}')   
                    # except(InvalidIndexError):
                             # print(f'PROBLEM {f}')
            else:
                df_new = dft.apply(calculate_impacts_gmt, dsi=dsi,  axis=1)#.compute(scheduler='processes')
