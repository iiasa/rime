    ...
    for variable in df.variable.unique():
        ...
        for ssp in SSPs:
            ...
            for region in regions:
                ...
                dfs = df.loc[(df.region==region) & (df.SSP==ssp) & (df.variable==variable)]
                if len(dfs)>0:
...
def calculate_impacts_gmt(dfx, dsi,  prefix_indicator='RCRE|', ssp_meta_col='ssp_family', gmt_below=1.5):
    '''
    Parameters
    ----------
    dfx : dask.DataFrame 
    ...
    prefix_indicator : str, optional
        DESCRIPTION. The default is 'RCRE|'. Use this to change the output indicator prefix.
...
# for x in range(18,len(dft)):
    # dfx = dft.iloc[x].squeeze()
    model = dfx['model']
    ...
    for indicator in list(dsi.data_vars):
        ...
        edf = pd.DataFrame(columns=pyam.IAMC_IDX) # dont have years because otherwise joins with empty rows
        ...
        try:
            agh = dsd.sel(year=tgt_y, gmt=tgt_g , method='nearest').to_dataframe(name=indicator)
            agh = agh.reset_index()
...
def map_transform_gmt(df1, mapdata, var_name, map_array=xr.Dataset(), gmt_name='threshold', temp_min=1.2, 
                      temp_max=3.5, drawdown_max=0.15, ):
    
    
            if len(df1.variable)>1:
                print('Error: more than 1 variable in DataFrame')
                breakss
            ...
            for model, scenario in df1.index:    
                ...
                if caution_checks:
                    ...
                    if gmt_values.max() > temp_max:
                        print(f'Warning! Max temperature above {temp_max}°C  {gmt_values.max()}, data thereafter not possible')
                        continue
                    if gmt_values.min() < temp_min:
                        print(f'Warning! Min temperature below {temp_min}°C  {gmt_values.min()}, data before not possible')
                ...
                new_thresholds = np.arange(mapdata.gmt.min(), mapdata.gmt.max() \
                ...
                data_interp["gmt"] = data_interp["gmt"].assign_coords(gmt=data_interp["gmt"].values)
                data_interp = data_interp.to_dataset()
                
                # Create new empty dataarray for the new data.
                map_array = map_array.assign(variables = {var_name: (('lat', 'lon', 'year'), np.full([len(data_interp.lat), len(data_interp.lon), len(years)], np.nan))})          
                data_interp[short][-1,:,:,] =  np.full([len(data_interp.lat), len(data_interp.lon)], np.nan)
                ...
                # Update map_array
                map_array[var_name][:, :, :len(years)] = updated_data.transpose('lat', 'lon', 'gmt')
...
def map_transform_gmt_multi_dask(df, mapdata,  gmt_name='threshold', use_dask=True,
                                 temp_min=1.2, temp_max=3.5, drawdown_max=0.15, ):
            ...
            # =============================================================================
                ...
                for model, scenario in df.index:
                    ...
                    if use_dask:
                        ...
                        delayed_map_transform = delayed(map_transform_gmt)(df1, mapdata, var_name, map_array)
                        delayed_tasks.append(delayed_map_transform)
                    else:
                        map_array = map_transform_gmt(df1, mapdata, var_name, map_array,)
                if use_dask:
                ...
                raise ValueError("Make sure that one of df or mapdata is only 1 scenario/indicator")
        ...
        map_array = map_array.assign_coords(coords={'lon': map_array.lon, 'lat': map_array.lat, 'year': years})
